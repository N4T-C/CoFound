...
export const Card = ({ children, className = "", noPadding = false, onClick, style, hoverable = false }) => (<div onClick={onClick} className={`
...
export const Chip = ({ label, color = "bg-cofound-grey/50 text-cofound-textLight dark:bg-white/5 dark:text-cofound-textDark/80", size = 'md', className = "" }) => (<span className={`
...
export const Avatar = ({ initials, size = "md", color = "bg-gradient-to-br from-cofound-blue to-cofound-darkBlue", className = "", image }) => {
    const sizes = {
        xs: "w-6 h-6 text-[9px]",
    ...
    return (<div className={`${sizes[size]} ${color} ${className} shrink-0 text-white rounded-full flex items-center justify-center font-bold font-mono shadow-sm border border-white/20 relative overflow-hidden ring-1 ring-black/5 dark:ring-white/10`}>
      {image ? (<img src={image} alt={initials} className="w-full h-full object-cover"/>) : (initials)}
    </div>);
};
export const Button = ({ variant = 'primary', size = 'md', children, className = '', fullWidth = false, ...props }) => {
    const variants = {
...
export const Skeleton = ({ className, variant = 'rect' }) => (<div className={`animate-pulse bg-cofound-grey/60 dark:bg-white/5 ${variant === 'circle' ? 'rounded-full' : 'rounded-lg'} ${className}`}></div>);
