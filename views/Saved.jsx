...
import { Bookmark, FileText, Briefcase, MoreHorizontal } from 'lucide-react';
import { SIGNALS, JOBS } from '../data';
const Saved = () => {
    const [activeTab, setActiveTab] = useState('ALL');
    return (<div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 border-b border-cofound-nav dark:border-white/10 pb-4">
         ...
         <div className="flex gap-2 p-1 bg-white dark:bg-cofound-darkCard border border-cofound-grey dark:border-white/10 rounded-lg">
            {['ALL', 'POSTS', 'JOBS'].map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${activeTab === tab
      ...
      <div className="space-y-4">
        ...
        {(activeTab === 'ALL' || activeTab === 'POSTS') && (<>
             ...
             {SIGNALS.slice(0, 2).map(signal => (<Card key={`saved-signal-${signal.id}`} className="flex gap-4 p-5 hover:border-cofound-blue/30 cursor-pointer group transition-all">
                   ...
                   <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2">
                         {signal.tags.map(tag => (<span key={tag} className="text-xs text-cofound-textLight dark:text-cofound-textDark/60 bg-cofound-bg dark:bg-white/5 px-2 py-0.5 rounded border border-transparent group-hover:border-cofound-grey dark:group-hover:border-white/10 transition-colors">#{tag}</span>))}
                      </div>
                   </div>
                </Card>))}
           </>)}
        ...
        {(activeTab === 'ALL' || activeTab === 'JOBS') && (<>
             ...
             {JOBS.slice(0, 2).map(job => (<Card key={`saved-job-${job.id}`} className="flex gap-4 p-5 hover:border-cofound-blue/30 cursor-pointer group transition-all">
        ...
        {activeTab === 'JOBS' && JOBS.length === 0 && (<div className="text-center py-12 text-cofound-textLight dark:text-cofound-textDark/50">
