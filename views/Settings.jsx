...
import { Bell, Moon, Shield, CreditCard, User, Lock, Check, AlertTriangle, Globe } from 'lucide-react';
import { Card, Avatar, Button, Chip } from '../components/UI';
const Toggle = ({ checked, onChange }) => (<button onClick={onChange} className={`w-11 h-6 flex items-center rounded-full p-1 transition-colors duration-200 ${checked ? 'bg-cofound-blue' : 'bg-cofound-grey dark:bg-white/10'}`}>
...
const SectionHeader = ({ icon: Icon, title, description }) => (<div className="flex items-start gap-4 mb-6">
...
const SettingsView = ({ currentTheme, onThemeChange }) => {
    const [emailDigest, setEmailDigest] = useState(true);
    const [marketingEmails, setMarketingEmails] = useState(false);
    const [twoFactor, setTwoFactor] = useState(true);
    return (<div className="max-w-3xl mx-auto space-y-8 animate-fade-in pb-20">
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-4">
           <div onClick={() => onThemeChange('light')} className={`cursor-pointer rounded-xl border-2 p-4 flex flex-col items-center gap-3 transition-all ${currentTheme === 'light' ? 'border-cofound-blue bg-blue-50/30 dark:bg-blue-500/5' : 'border-cofound-grey dark:border-white/10 hover:border-cofound-blue/50'}`}>
           ...
           <div onClick={() => onThemeChange('dark')} className={`cursor-pointer rounded-xl border-2 p-4 flex flex-col items-center gap-3 transition-all ${currentTheme === 'dark' ? 'border-cofound-blue bg-blue-50/30 dark:bg-blue-500/5' : 'border-cofound-grey dark:border-white/10 hover:border-cofound-blue/50'}`}>
