...
const Chat = () => {
    const [selectedThread, setSelectedThread] = useState(CHAT_THREADS[0]);
    ...
    return (<Card className="h-[calc(100vh-120px)] flex flex-row !p-0 overflow-hidden shadow-lg border-cofound-nav dark:border-white/10">
      ...
      <div className="w-80 border-r border-cofound-nav dark:border-white/10 flex flex-col bg-white dark:bg-cofound-darkCard hidden md:flex">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
           {CHAT_THREADS.map(thread => (<div key={thread.id} onClick={() => setSelectedThread(thread)} className={`p-4 cursor-pointer transition-colors border-l-2 ${selectedThread.id === thread.id ? 'bg-blue-50/40 dark:bg-blue-500/10 border-cofound-blue' : 'border-transparent hover:bg-cofound-bg dark:hover:bg-white/5'}`}>
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-4">
           {messages.map(msg => (<div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] md:max-w-[70%] ${msg.sender === 'me' ? 'items-end' : 'items-start'} flex flex-col`}>
                   <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed shadow-sm relative group ${msg.sender === 'me'
        ...
        <div className="p-4 bg-white dark:bg-cofound-darkCard border-t border-cofound-nav dark:border-white/10 shrink-0">
           <div className="flex items-end gap-2">
              ...
              <div className="flex-1 bg-cofound-bg dark:bg-white/5 rounded-2xl p-2 flex items-center gap-2 border border-transparent focus-within:border-cofound-blue/30 focus-within:bg-white dark:focus-within:bg-black/20 transition-all">
                 <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), sendMessage())} placeholder="Type a message..." className="flex-1 bg-transparent border-none outline-none text-sm text-cofound-text dark:text-white placeholder:text-cofound-textLight/70 dark:placeholder:text-white/30 px-2"/>
              ...
              <Button onClick={sendMessage} size="md" variant={inputText.trim() ? "primary" : "secondary"} className={`!p-2.5 rounded-full mb-0.5 transition-all`} disabled={!inputText.trim()}>
