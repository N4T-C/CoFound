...
import { MapPin, IndianRupee, Zap, Bookmark, Share2 } from 'lucide-react';
const Jobs = () => {
    return (<div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-12">
      ...
      <div className="space-y-5">
        {JOBS.map((job, idx) => (<Card key={job.id} hoverable className="group relative transition-all duration-300" style={{ animationDelay: `${idx * 100}ms` }}>
            ...
            <div className="flex items-center justify-between border-t border-cofound-grey/60 dark:border-white/5 pt-5 mt-5 pl-0 md:pl-[84px]">
              <div className="flex flex-wrap items-center gap-2">
                 {job.tags.map(tag => (<span key={tag} className="px-3 py-1 bg-cofound-bg dark:bg-white/5 border border-transparent hover:border-cofound-grey dark:hover:border-white/10 text-cofound-textLight dark:text-cofound-textDark/70 rounded-lg text-xs font-bold transition-colors">
                     {tag}
                   </span>))}
              </div>
